...
function managerRetry(self, key)
{
	...
	/*
	 * If we've already called back
	 * for this request, don't retry.
	 */
	if (options.callbackFired) {
		options.retries = 0;
...
function managerRecv(self, msg, peer)
{
	...
	case Protocol.PDU_GetResponse:
		...
		switch (error) {
		case Protocol.ERROR_NoError:
			break;
		case Protocol.ERROR_NoSuchName:
			options.callbackFired = true;
			options.cb(new Error('no such name'), null);
			return;
		default:
			options.callbackFired = true;
			options.cb(error, null);
			return;
		}
...
function managerWalk(agent, oid, cb, options)
{
	if (typeof(agent) !== 'string')
		throw new TypeError('agent (string) required');
	if (typeof(oid) !== 'string')
		throw new TypeError('oid (string) required');
	...
	var walk = { position: 1, oidBase: oid, results: [] };
	var next = function(err, res) {
		var thisOid = walk.oidBase + '.' + walk.position;
		if (!err && !res) {
			manager.get(agent, thisOid, next, options);
			return;
		}
		if (err) {
			if (err.message === 'no such name') {
				// walk is finished
				cb(null, walk.results);
			} else {
				// unexpected error
				cb(err, null);
			}
			...
			cb(new Error('missing result value'));
			return;
		}
		// save the result and continue walking
		walk.results.push(res[thisOid].value);
		walk.position++;
		next();
	};
	next();
...
Manager.prototype.walk = managerWalk;
